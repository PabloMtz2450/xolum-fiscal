...
export type FinkokCancellationMaterial = {
  certificatePemBase64: string;
  privateKeyDes3PemBase64: string;
...
export type FinkokPendingStatus = {
  status?: string;
  uuid?: string;
  uuidStatus?: 'S' | 'F' | string;
  nextAttempt?: string;
  attempts?: string;
  error?: string;
  date?: string;
...
function unwrapResult(response: unknown, preferredKeys: string[]): Record<string, any> {
  ...
  for (const key of preferredKeys) {
    if (current[key] != null) current = firstObject(current[key]);
  }
  return current;
}
...
function stampFailure(result: Record<string, any>): PacRejection {
  ...
  return {
    providerCode: String(result.CodEstatus ?? result.codEstatus ?? 'FINKOK-STAMP-UNKNOWN'),
    message: String(result.CodEstatus ?? result.codEstatus ?? 'Finkok no devolvió un estado reconocible de timbrado.'),
    retryable: false,
  };
}
...
export class FinkokPacAdapter implements PacAdapter {
  ...
  async healthcheck(): Promise<{ ok: boolean; detail?: string }> {
    try {
      await this.clientFactory(WSDL[this.environment].stamp);
      await this.clientFactory(WSDL[this.environment].cancel);
      return { ok: true };
    } catch (error) {
  ...
  async preflight(xml: string): Promise<PacRejection[]> {
    ...
    if (size >= MAX_XML_BYTES) {
      errors.push({ providerCode: 'FINKOK-XML-SIZE', message: 'Finkok limita cada XML a menos de 1 MB.', retryable: false });
    }
    if (!xml.trimStart().startsWith('<?xml') && !xml.trimStart().startsWith('<cfdi:Comprobante')) {
      errors.push({ providerCode: '301', message: 'El XML no tiene una estructura inicial reconocible.', retryable: false });
    }
    if (!/\bSello="[^"]+"/.test(xml) || !/\bCertificado="[^"]+"/.test(xml) || !/\bNoCertificado="\d{20}"/.test(xml)) {
      errors.push({ providerCode: 'CFDI40102', message: 'El XML debe llegar a Finkok con Sello, Certificado y NoCertificado completos.', retryable: false });
    }
    return errors;
  }
  ...
  async stamp(request: PacStampRequest): Promise<PacStampResult> {
    ...
    try {
      const client = await this.clientFactory(WSDL[this.environment].stamp);
      const args = {
        xml: Buffer.from(request.xml, 'utf8').toString('base64'),
        username: this.credentials.username,
        password: this.credentials.password,
      };
      const response = await (client as any).stampAsync(args);
      ...
      if (!uuid || !stampedXml) {
        return { ok: false, provider: this.provider, rejection: { providerCode: 'FINKOK-INCOMPLETE-STAMP', message: 'Finkok reportó timbrado exitoso pero no devolvió UUID/XML completos.', retryable: false } };
      }
      return {
        ok: true,
        uuid,
        ...
        stampedAt: String(result.Fecha ?? result.fecha ?? new Date().toISOString()),
      };
    } catch (error) {
  ...
  async cancel(input: { uuid: string; reason: '01'|'02'|'03'|'04'; replacementUuid?: string; issuerRfc: string }): Promise<{ requestId: string; status: string; rawCode?: string }> {
    ...
    const client = await this.clientFactory(WSDL[this.environment].cancel);
    ...
    const args = {
      ...
      cer: this.cancellation.certificatePemBase64,
      key: this.cancellation.privateKeyDes3PemBase64,
      store_pending: this.cancellation.storePending ?? false,
    };
    const response = await (client as any).cancelAsync(args);
    ...
    const requestId = String(result.RfcEmisor ?? input.issuerRfc) + ':' + input.uuid;
    return { requestId, status, rawCode };
  }

  async status(input: { uuid: string; issuerRfc: string; receiverRfc?: string; total?: number }): Promise<{ satStatus: string; cancellable?: string; cancellationStatus?: string }> {
    if (!input.receiverRfc || input.total == null) throw new Error('FINKOK_STATUS_REQUIRES_RECEIVER_RFC_AND_TOTAL');
    const client = await this.clientFactory(WSDL[this.environment].cancel);
    const response = await (client as any).get_sat_statusAsync({
      username: this.credentials.username,
      password: this.credentials.password,
      taxpayer_id: input.issuerRfc,
  ...
  async queryPending(uuid: string): Promise<FinkokPendingStatus> {
    const client = await this.clientFactory(WSDL[this.environment].stamp);
    const response = await (client as any).query_pendingAsync({
      username: this.credentials.username,
      password: this.credentials.password,
      uuid,
    });
    ...
    return {
      ...
      xml: normalizeXml(result.xml),
      uuid: result.uuid,
      uuidStatus: result.uuid_status,
      nextAttempt: result.next_attempt,
