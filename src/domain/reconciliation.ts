...
export function suggestReconciliation(movement: BankMovement, receivables: Receivable[]): ReconciliationSuggestion[] {
  const candidates = receivables.map((receivable) => {
    let confidence = 0;
    ...
    if (movement.senderRfc && movement.senderRfc.length >= 12) { confidence += 5; reasons.push('RFC disponible para revisión'); }

    const mode: MatchMode = confidence >= 55 ? 'ASSISTED' : 'MANUAL';
    return { movementId:movement.id, receivableId:receivable.id, confidence, mode, reasons, amountMatches, referenceMatches };
  }).sort((a,b) => b.confidence - a.confidence);

  // Sólo un candidato inequívoco por importe + referencia puede automatizarse.
  const eligible = candidates.filter((candidate) => candidate.amountMatches && candidate.referenceMatches && candidate.confidence >= 90);
